...
class _MasterViewState extends State<MasterView>
    ...
    return Column(
      children: [
        ...
        Container(
          ...
          child: Row(
            children: [
              _vehicleStat('Total', vehicles.length, const Color(0xFF1E3A8A)),
              ...
              _vehicleStat(
                ...
                vehicles
                    .where(
                      (v) => (v as FleetVehicle).status == VehicleStatus.idle,
                    )
                    .length,
                const Color(0xFFF59E0B),
              ),
              ...
              _vehicleStat(
                ...
                vehicles
                    .where(
                      (v) => (v as FleetVehicle).status == VehicleStatus.alert,
                    )
                    .length,
                const Color(0xFFEF4444),
              ),
              ...
              builder: (_) {
                ...
                if (_vehicleSearchQuery.isNotEmpty) {
                  filtered = filtered.where((v) {
                    final vehicle = v as FleetVehicle;
                    return vehicle.registration.toLowerCase().contains(
                          _vehicleSearchQuery,
                        ) ||
                        vehicle.driverName.toLowerCase().contains(
                          _vehicleSearchQuery,
                        );
                  }).toList();
                  // Sort: exact prefix matches first, then contains matches, alphabetically within each group
                  filtered.sort((a, b) {
                    final aReg = (a as FleetVehicle).registration.toLowerCase();
                    final bReg = (b as FleetVehicle).registration.toLowerCase();
                    final aStarts = aReg.startsWith(_vehicleSearchQuery)
                        ? 0
                        : 1;
                    final bStarts = bReg.startsWith(_vehicleSearchQuery)
                        ? 0
                        : 1;
                    if (aStarts != bStarts) return aStarts.compareTo(bStarts);
                  ...
                  // Default sort: alphabetically by registration
                  filtered.sort(
                    (a, b) => (a as FleetVehicle).registration.compareTo(
                      (b as FleetVehicle).registration,
                    ),
                  );
                }
                return ListView.builder(
                  ...
                  itemCount: filtered.length,
                  itemBuilder: (_, i) {
                    final v = filtered[i] as FleetVehicle;
  ...
  void _submitEnrollment(
    ...
    final conds = faceConditions.entries
        .where((e) => e.value)
        .map((e) => e.key)
        .toList();
    ...
    final driver = MasterDriver(
      ...
      shift: selectedShift,
      enrollment: facePhotos.length >= conds.length && conds.isNotEmpty
          ? 'ENROLLED'
          : 'PARTIAL',
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Text(
                    '$total photo(s) uploaded across ${photos.length} condition(s)',
                    style: GoogleFonts.plusJakartaSans(
                      fontSize: 12,
                      fontWeight: FontWeight.w500,
                                      ...
                                      : Container(
                                          ...
                                          child: DropdownButtonHideUnderline(
                                            child: DropdownButton<UnassignedDevice>(
                                              ...
                                              selectedItemBuilder: (context) => unassignedDevices
                                                  .map(
                                                    (d) => Row(
                                                      children: [
                                                        const Icon(
                                              ...
                                              items: unassignedDevices
                                                  .map(
                                                    (
                                                      d,
                                                    ) => DropdownMenuItem<UnassignedDevice>(
                                                      value: d,
                                                      child: Column(
                                                        crossAxisAlignment:
                                                            CrossAxisAlignment
                                    ...
                                    if (formKey.currentState!.validate()) {
                                      ...
                                      fleet.addVehicle(
                                        ...
                                        tabletModel:
                                            selectedDevice?.deviceModel ?? '',
                                        overspeedThreshold:
                                            int.tryParse(overspeedCtrl.text) ??
                                            60,
                                        reVerificationInterval:
                                            int.tryParse(
                                              reVerification.split(' ')[0],
                                            ) ??
                                            2,
                                        frontCameraOperational:
                                            preChecks['Front camera operational'] ??
                                            false,
                                        roadFacingCameraOperational:
                                            preChecks['Road-facing camera operational'] ??
                                            false,
                                        gpsSignalConfirmed:
                                            preChecks['GPS signal confirmed'] ??
                                            false,
                                        mdmKioskModeActive:
                                            preChecks['MDM/Kiosk mode active'] ??
                                            false,
                                        appInstalledAndSigned:
                                            preChecks['App installed & signed'] ??
                                            false,
                                        mountSecureNoTamper:
                                            preChecks['Mount secure, no tamper'] ??
                                            false,
                                      );
  ...
  InputDecoration _inputDec(String hint, IconData icon) => InputDecoration(
    ...
    filled: true,
    fillColor: AppTheme.of(context).isDark
        ? const Color(0xFF1E293B)
        : const Color(0xFFF8FAFC),
    isDense: true,
  ...
  void _showVehicleDetailsSheet(BuildContext context, FleetVehicle vehicle) {
    ...
    final List drivers = raw['assignedDrivers'] is List
        ? raw['assignedDrivers']
        : [];
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: GoogleFonts.plusJakartaSans(
                      fontSize: 14,
                      fontWeight: FontWeight.w500,
                      color: color ?? AppTheme.of(context).textPrimary,
                    ),
                  ),
                ],
              ),
                    ...
                    Row(
                      children: [
                        Container(
                          padding: const EdgeInsets.all(12),
                          decoration: BoxDecoration(
                            color: const Color(0xFF2B72F5).withOpacity(0.1),
                            borderRadius: BorderRadius.circular(12),
                          ),
                          child: const Icon(
                            Icons.local_shipping_rounded,
